...
func ConsumeAchievementStream(githubService *GithubService) {
	ctx := context.Background()
	streamName := cmd.AutomaticEvents
	groupName := "achievement-group"
	consumerName := "achievement-consumer-1"
	...
	for {
		...
		for _, stream := range streams {
			for _, message := range stream.Messages {
				...
				if err != nil {
					log.Printf("Error unmarshalling achievement: %v", err)
					continue
				}

				log.Printf("Received achievement: %+v", achievement)

				// TODO: Take appropriate action with the achievement
